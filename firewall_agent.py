...
class Agent:
    def __init__(
        ...
        self.country_cache = CountryCIDRCache(cidr_feed_base, cache_root / "country", logger)
        self.backend: FirewallBackend
        if is_windows():
            self.backend = WindowsFirewallBackend(logger)
        elif is_linux():
            self.backend = LinuxFirewallBackend(logger)
        else:
